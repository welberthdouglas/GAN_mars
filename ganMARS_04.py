...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 26 12:40:29 2020
...
def define_gan(g_model, d_model):
    ...
    #opt = Adam(lr=0.0002, beta_1=0.5)
    model.compile(loss='mse', optimizer=g_optimizer)
...
#    
#    X = (expand_dims(np.array([cv2.imread(i,0) for i in [path+i for i in img_df.file[img_df['class']!=0]]]),axis = -1)-127.5)/127.5
#    X = X.astype('float32')
#    #X = X[0:10000]
#    return X
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=30, n_batch=156):
    ...
    for i in range(n_epochs):
        ...
        for j in range(bat_per_epo):
            ...
            # train discriminator
            dis_loss_real,_ = d_model.train_on_batch(X_real, y_real)
            dis_loss_fake,_ = d_model.train_on_batch(X_fake, y_fake)
...
def summarize_performance(epoch, g_model, d_model, dataset, latent_dim, n_samples=100,n_plots=16):
